...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="javascript:;" data-target="kelas" onclick="load_page({'href':base_url+'peserta/daftar/'+$('#kel_id').val()});return false">Peserta Diklat</a></li>
        <li class="active">Daftar Hadir Peserta</li>
    </ol>
</section>
            ...
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <div class="pull-left margin-r-5">
                        <a data-toggle="tooltips" title="Tambah tanggal" onclick="do_action(this);return false" data-action="add_data" class="btn-sm btn btn-primary"><i class="fa fa-plus"></i> Tambah Tanggal</a>
                        <a data-toggle="tooltips" title="Cetak daftar hadir" data-type="hadir" onclick="print_data(this);return false" class="btn-sm btn btn-success btn-delete disabled"><i class="fa fa-print"></i> Cetak Daftar Hadir</a>
                        <a data-toggle="tooltips" title="Cetak gambar daftar hadir" data-type="gambar" onclick="print_data(this);return false" class="btn-sm btn btn-success btn-delete disabled"><i class="fa fa-print"></i> Cetak Gambar</a>
                        <a data-toggle="tooltips" title="Upload kehadiran" onclick="do_action(this);return false" data-action="upload" class="btn-sm btn-delete disabled btn btn-info"><i class="fa fa-upload"></i> Upload Daftar Hadir</a>
                        <a data-toggle="tooltips" title="Hapus tanggal" onclick="bulk_delete();return false" class="btn-sm btn-delete disabled btn btn-danger"><i class="fa fa-trash"></i> Hapus Tanggal</a>
                    </div>
                    <div class="pull-left margin-r-5">
...
<script>
    ...
    function print_data(ob) {
        ...
        if (dtlen == 0){
            ...
            if (tipe == 'hadir'){
                var url = base_url + 'peserta/cetak_hadir/'+data;
            } else {
                var url = base_url + 'peserta/cetak_gambar_hadir/'+data;
            }
    ...
    function do_action(ob) {
        ...
        if (!act){
            ...
            var url = base_url + 'peserta/add_hadir/' + kel_id;
            var dt = {'href':url,'title':'Tambah tanggal kehadiran peserta'}
            show_modal(dt);
        } else if (act == 'upload'){
            ...
            if (dh_id.length == 0){
                ...
                var url     = base_url + 'peserta/upload_hadir/' + dh_id.val();
    ...
    function bulk_delete() {
        ...
        if (len == 0){
            ...
            $('.overlay').show();
            $.ajax({
                url     : base_url + 'peserta/bulk_delete_hadir',
                type    : 'POST',
                dataType: 'JSON',
    ...
    function delete_data(ob) {
        ...
        if (!id){
            ...
            $('.overlay').show();
            $.ajax({
                url     : base_url + 'peserta/delete_hadir',
                type    : 'POST',
                dataType: 'JSON',
                data    : { id : id },
                success : function (dt) {
    ...
    function load_table() {
        ...
        var kel_id   = $('#kel_id').val();
        $.ajax({
            url     : base_url + 'peserta/data_hadir',
