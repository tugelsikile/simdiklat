<form id="modalForm" class="form" autocomplete="off">
    ...
    <div class="form-group col-md-4">
        <label for="dh_format">Format Kehadiran</label>
        <select id="dh_format" name="dh_format" style="width: 100%">
            <option value="pagi">Pagi - Siang</option>
            <option value="siang">Pagi - Siang - Malam</option>
            <option value="malam">Siang - Malam</option>
        </select>
    </div>
...
<script>
    ...
    $('#modalForm').submit(function () {
        ...
        $.ajax({
            url     : base_url + 'peserta/add_hadir_submit',
            type    : 'POST',
            dataType: 'JSON',
            data    : $(this).serialize(),
            success : function (dt) {
