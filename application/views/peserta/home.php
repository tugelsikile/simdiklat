        ...
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
                <div class="pull-left margin-r-5">
                    <a data-toggle="tooltips" title="Tambah peserta" onclick="do_action(this);return false" data-action="add_data" class="btn-sm btn btn-primary"><i class="fa fa-plus"></i> Tambah Peserta</a>
                    <a data-toggle="tooltips" title="Import peserta" onclick="do_action(this);return false" data-action="import" class="btn-sm btn btn-primary"><i class="fa fa-upload"></i> Import Peserta</a>
                    <a data-toggle="tooltips" title="Download peserta" onclick="do_action(this);return false" data-action="download" class="btn-sm btn btn-primary"><i class="fa fa-download"></i> Download Peserta</a>
                    <a data-toggle="tooltips" title="Daftar hadir" onclick="load_page({'href':base_url+'peserta/hadir/'+$('#kel_id').val()})" href="javascript:;" class="btn-sm btn btn-primary"><i class="fa fa-calendar-check-o"></i> Daftar Hadir</a>
                    <a data-toggle="tooltips" title="Cetak biodata" onclick="print_data(this);return false" class="btn-sm btn-delete disabled btn btn-info"><i class="fa fa-print"></i> Cetak Biodata</a>
                    <a data-toggle="tooltips" title="Hapus peserta" onclick="bulk_delete();return false" class="btn-sm btn-delete disabled btn btn-danger"><i class="fa fa-trash"></i> Hapus Peserta</a>
                </div>
                <div class="pull-left margin-r-5">
...
<script>
    ...
    function do_action(ob) {
        ...
        if (!act){
            ...
            var url = base_url + 'peserta/add_data/' + kel_id;
            var dt = {'href':url,'title':'Tambah data peserta'}
            show_modal(dt);
        } else if (act == 'import'){
            var url     = base_url + 'peserta/import/' + kel_id;
    ...
    function bulk_delete() {
        ...
        if (len == 0){
            ...
            $('.overlay').show();
            $.ajax({
                url     : base_url + 'peserta/bulk_delete',
                type    : 'POST',
                dataType: 'JSON',
    ...
    function delete_data(ob) {
        ...
        var kon = confirm('Anda yakin ingin menghapus peserta ini ?');
        if (!id){
            show_msg('Pilih data','error');
        } else if (kon){
            $('.overlay').show();
            $.ajax({
                url     : base_url + 'peserta/delete_data',
                type    : 'POST',
                dataType: 'JSON',
                data    : { id : id },
                success : function (dt) {
    ...
    function load_table() {
        ...
        var kel_id   = $('#kel_id').val();
        $.ajax({
            url     : base_url + 'peserta/data_home',
