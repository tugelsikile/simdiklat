        ...
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
                <div class="pull-left margin-r-5">
                    <a data-toggle="tooltip" title="Tambah panitia" onclick="do_action(this);return false" data-action="add_data" class="btn-sm btn btn-primary"><i class="fa fa-plus"></i> Tambah Panitia</a>
                    <a data-toggle="tooltip" title="Daftar Hadir" onclick="load_page({'data-target':'kelas','href':base_url+'panitia/hadir/'+$('#kel_id').val()});return false" class="btn btn-sm btn-primary"><i class="fa fa-calendar-check-o"></i> Daftar Hadir</a>
                    <a data-toggle="tooltip" title="Hapus panitia" onclick="bulk_delete();return false" class="btn-sm btn-delete disabled btn btn-danger"><i class="fa fa-trash"></i> Hapus Panitia</a>
                </div>
            </div>
        </div>
...
<script>
    ...
    function do_action(ob) {
        ...
        if (!act){
            ...
            var url = base_url + 'panitia/add_data/' + kel_id;
            var dt = {'href':url,'title':'Tambah data panitia'}
            show_modal(dt);
        } else if (act == 'import'){
            var url     = base_url + 'panitia/import/' + kel_id;
            var dt      = {'href':url, 'title':'Import data panitia'}
            show_modal(dt);
        }
    }
    function bulk_delete() {
        ...
        if (len == 0){
            ...
            $('.overlay').show();
            $.ajax({
                url     : base_url + 'panitia/bulk_delete',
                type    : 'POST',
                dataType: 'JSON',
    ...
    function delete_data(ob) {
        ...
        if (!id){
            ...
            $('.overlay').show();
            $.ajax({
                url     : base_url + 'panitia/delete_data',
                type    : 'POST',
                dataType: 'JSON',
                data    : { id : id },
                success : function (dt) {
    ...
    function load_table() {
        ...
        var kel_id   = $('#kel_id').val();
        $.ajax({
            url     : base_url + 'panitia/data_home',
