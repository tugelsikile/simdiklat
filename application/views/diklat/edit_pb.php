<form id="modalForm" class="form" autocomplete="off">
    <input type="hidden" name="pb_id" value="<?php echo $data->pb_id;?>">
    <div class="form-group col-md-12">
        <label for="pb_name">Nama Pusat Belajar</label>
        <input type="text" id="pb_name" name="pb_name" class="form-control" autocomplete="nope" value="<?php echo $data->pb_name;?>">
    </div>
    <div class="form-group col-md-6">
        <label for="pb_phone">No. Telp</label>
        <input type="text" id="pb_phone" name="pb_phone" class="form-control" autocomplete="nope" value="<?php echo $data->pb_phone;?>">
    </div>
    <div class="form-group col-md-6">
        <label for="pb_email">Email</label>
        <input type="text" id="pb_email" name="pb_email" class="form-control" autocomplete="nope" value="<?php echo $data->pb_email;?>">
    </div>
    <div class="form-group col-md-12">
        <label for="pb_address">Alamat Pusat Belajar</label>
        <textarea name="pb_address" id="pb_address" class="form-control"><?php echo $data->pb_address;?></textarea>
    </div>
    <div class="form-group col-md-4">
        ...
        <select name="kec_id" id="kec_id" style="width: 100%" class="form-control">
            <?php
            foreach ($kec as $val){
                echo '<option value="'.$val->kec_id.'">'.$val->name.'</option>';
            }
            ?>
        </select>
    </div>
    <div class="form-group col-md-4">
        ...
        <select name="kab_id" id="kab_id" style="width: 100%" class="form-control" onchange="kab_selected()">
            <?php
            foreach ($kab as $val){
                echo '<option value="'.$val->kab_id.'">'.$val->name.'</option>';
            }
            ?>
        </select>
    </div>
...
<script>
    $('#kec_id').val('<?php echo $data->kec_id;?>');
    $('#kab_id').val('<?php echo $kab_id;?>');
    $('#prov_id').val('<?php echo $prov_id;?>');
    ...
    $('#modalForm').submit(function () {
        ...
        $.ajax({
            url     : base_url + 'diklat/edit_pb_submit',
            type    : 'POST',
            dataType: 'JSON',
            data    : $(this).serialize(),
            success : function (dt) {
