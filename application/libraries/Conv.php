...
class Conv {
    ...
    function genPass($length=FALSE,$alpa=FALSE,$lower=FALSE){
        if (!$length){ $length 	= 8; }
        if (!$alpa){ $alpa = 'alpha'; }
        switch ($alpa){
            case 'alpha'    : $characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'; break;
            case 'numeric'  : $characters = '0123456789'; break;
            case 'alnum'    : $characters = '0123456789ABCDEFGHJKLMNPRSTUVWXY'; break;
        }
        $string 	= "";
        for ($p = 0; $p < $length; $p++) {
